#!/usr/bin/env python3
"""
Dakota driver for hydroRaVENS calibration — W02 (independent recession exponents).
"""
...
def _tile_list():
    if not any(n in _param_cfg for n in _TILE_NAMES[:N_RESERVOIRS]):
        return None
    vals = [get(n) if n in _param_cfg else 0.0 for n in _TILE_NAMES[:N_RESERVOIRS]]
...
def _recession_exponents():
    """Independent b for soil and karst; shallow stays linear."""
    return [1.0, get('recession_b_soil'), get('recession_b_karst')]
...
try:
    result = run_and_score(
        ...
        recession_exponents_calibrated = 2,
        modules               =  MODULES,
